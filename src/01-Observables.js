...
miObs1.forEach(console.log.bind(console))
...
const clicks = Observable.fromEvent(button, 'click') // !~ remplaza por el evento adecuado y logea
...
//const repos = Observable.fromPromise(reposDeUnUsuario('burabure'))
//  .map(repos => repos.map(repo => repo.full_name)) // !~ Por cada respuesta retorna un array con los 'full_name' de los repositorios


const reposEnClick = clicks.flatMap(click => repos) // !~ Por cada 'click' mandamos las 'repos' (Ojo! esto es un observable de observables)
  .forEach(console.log.bind(console)) // !~ logea a consola
